...
double Lane::LaneSpeed(std::vector<vector<double>> sensor_fusion, double car_s, double end_path_s, double end_path_d)
{
	...
	for (int i = 0; i < sensor_fusion.size(); i++)
	{
		...
		if ((sense_d > 4 * lane_number) && (sense_d < 4 * (lane_number + 1)))
		{
			if ((sense_s - end_path_s) > 0.0)
			{
				...
				double lane_speed = sqrt(pow(vx, 2) + pow(vy, 2));
			}
		}
	}
	return(lane_speed);
