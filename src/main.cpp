      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
			...
			//in miles per hour
			double path_velocity = 40.0;

			double N = target_dist / (0.02*(path_velocity / 2.25));
			double x_add_on = target_dist / N;
			//std::cout << x_add_on;
			...
			double x_point_prev = ref_x;
			double y_point_prev = ref_y;
			...
			{
				double x_point = x_add_on*(i + 1);
				double y_point = s(x_point);

				//rotate back to global coordinates.
				x_point = x_point*cos(ref_yaw) - y_point*sin(ref_yaw);
				y_point = y_point*cos(ref_yaw) + x_point*sin(ref_yaw);
