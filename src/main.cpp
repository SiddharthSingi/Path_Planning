          ...
          // j[1] is the data JSON object
			...
			if (ref_vel < 40 * 4 / 9 && car_close == true)
			{
				...
				for (int j = 0; j < possible_shifts.size(); j++)
				{
					...
					if (possible_shifts[j] == true && (lane_speeds[j]>max_lane_speed))
					{
			...
			*/
			...
			//If for some reason that is not true than do not add any points to the 
			//next_x_vals at all.
			...
			for (int k = 2; k < ptsx.size(); k++)
			{
				if (ptsx[k] < 0)
				{
					ptsx.erase(ptsx.begin() + k + 1);
					ptsy.erase(ptsy.begin() + k + 1);
